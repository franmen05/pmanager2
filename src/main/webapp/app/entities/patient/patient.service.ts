...
export class PatientService {
  ...
  protected convertDateFromClient(patient: IPatient): IPatient {
    const copy: IPatient = Object.assign({}, patient, {
      birthDate: patient.birthDate && patient.birthDate.isValid() ? patient.birthDate.toJSON() : undefined,
      createDate: patient.createDate && patient.createDate.isValid() ? patient.createDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((patient: IPatient) => {
        ...
        patient.createDate = patient.createDate ? moment(patient.createDate) : undefined;
      });
    }
    return res;
  }
